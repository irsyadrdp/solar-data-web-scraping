### Website Scrapping - Solar Winds MAG
...
createTable <- function(data){
    i<-20 #initiate iteration
    
    #initiate matrix table
    MAG<-matrix(ncol=13)
    #naming header
    colnames(MAG)<-c("year","month","date","time(HHMM)","modified julian day","seconds of the day","S","Bx","By","Bz","Bt","Lat.","Long.") 
    
    repeat { 
        i<-i+1 #iteration
        temp<-unlist(strsplit(data[[i]], "\\s+"))  #split string
        MAG<-rbind(MAG, c(temp[1],temp[2],temp[3],temp[4],temp[5],temp[6],temp[7],temp[8],temp[9],temp[10],temp[11],temp[12],temp[13]))    #append data found to matrix table
